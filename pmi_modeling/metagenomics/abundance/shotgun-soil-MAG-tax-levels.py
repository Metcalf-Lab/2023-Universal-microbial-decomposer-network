...
#############################################################################################################
print("SPECIES")
allShotgun = ca.read_amplicon('MAG_species_table_final.biom','shotgun_metadata_q2.txt', min_reads=0, normalize = None)
...
Shotgun_soil = Shotgun_soil.filter_abundance(cutoff=1)
...
with open('MAG_species_allpred_output.csv', 'w') as f:
...
#############################################################################################################
print("GENUS")
allShotgun = ca.read_amplicon('MAG_genus_table_final.biom','shotgun_metadata_q2.txt', min_reads=0, normalize = None)
...
with open('MAG_genus_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
...
#############################################################################################################
print("FAMILY")
allShotgun = ca.read_amplicon('MAG_family_table_final.biom','shotgun_metadata_q2.txt', min_reads=0, normalize = None)
...
with open('MAG_family_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
...
#############################################################################################################
print("ORDER")
allShotgun = ca.read_amplicon('MAG_order_table_final.biom','shotgun_metadata_q2.txt', min_reads=0, normalize = None)
...
with open('MAG_order_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
...
#############################################################################################################
print("CLASS")
allShotgun = ca.read_amplicon('MAG_class_table_final.biom','shotgun_metadata_q2.txt', min_reads=0, normalize = None)
...
with open('MAG_class_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
...
#############################################################################################################
print("PHYLUM")
allShotgun = ca.read_amplicon('MAG_phylum_table_final.biom','shotgun_metadata_q2.txt', min_reads=0, normalize = None)
...
with open('MAG_phylum_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
