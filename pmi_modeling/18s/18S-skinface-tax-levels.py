...
all_18S.sample_metadata.host_subject_id.value_counts()
all_18S.sample_metadata.sample_site.value_counts()
...
all_18S_filtered = all_18S.filter_samples('sample_site', 'skin.face')
all_18S_filtered = all_18S_filtered.filter_abundance(min_abundance=1)
all_18S_filtered.sample_metadata.sample_type.value_counts()

print("Number of bodies: ",len(all_18S_filtered.sample_metadata.host_subject_id.value_counts()))


# # General Model using ADD 0C as Response and Adding Metadata as Features


all_18S_filtered_meta = all_18S_filtered.add_sample_metadata_as_features(['facility'])
print("Number of features: ",len(all_18S_filtered_meta.feature_metadata))



# designate data
X = all_18S_filtered_meta.data
y = all_18S_filtered_meta.sample_metadata['add_0c']
y = (y.astype(float))

# group by body
groups = all_18S_filtered_meta.sample_metadata['host_subject_id']
...
with open('18S_skin_face_species_allpred_output.csv', 'w') as f:
...
#############################################################################################################
print("GENUS")
all_18S = ca.read_amplicon('genus-normalized-table.biom','combined-metadata-simple-nov2020.txt', min_reads=1, normalize = None)
...
# loops through the sets of training and test ids in the outer loop
# the number of loops will match the number of folds as each fold is used
with open('18S_skin_face_genus_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
...
#############################################################################################################
print("FAMILY")
all_18S = ca.read_amplicon('family-normalized-table.biom','combined-metadata-simple-nov2020.txt', min_reads=1, normalize = None)
...
# loops through the sets of training and test ids in the outer loop
# the number of loops will match the number of folds as each fold is used
with open('18S_skin_face_family_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
...
#############################################################################################################
print("ORDER")
all_18S = ca.read_amplicon('order-normalized-table.biom','combined-metadata-simple-nov2020.txt', min_reads=1, normalize = None)
...
# loops through the sets of training and test ids in the outer loop
# the number of loops will match the number of folds as each fold is used
with open('18S_skin_face_order_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
...
#############################################################################################################
print("CLASS")
all_18S = ca.read_amplicon('class-normalized-table.biom','combined-metadata-simple-nov2020.txt', min_reads=1, normalize = None)
...
# loops through the sets of training and test ids in the outer loop
# the number of loops will match the number of folds as each fold is used
with open('18S_skin_face_class_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
...
#############################################################################################################
print("PHYLUM")
all_18S = ca.read_amplicon('phylum-normalized-table.biom','combined-metadata-simple-nov2020.txt', min_reads=1, normalize = None)
...
# loops through the sets of training and test ids in the outer loop
# the number of loops will match the number of folds as each fold is used
with open('18S_skin_face_phylum_allpred_output.csv', 'w') as f:
	print("SampleID,true_add,predicted_add",file=f)
	for train_ids, test_ids in outer_cv:
#     inner_cv creates 35 folds by leave-one-body-out for hyperparamter tuning
#     uses only the train ids present in the current outer loop fold which is one less body since the outer loop
