...
for j = 1:numel(param2get)
    ...
    if strcmp(param2get{j}, 'directory')
        ...
        while exist(varargout{j}, 'file')
            k = k + 1;
            varargout{j} = [varargout{j}.directory num2str(k)];
        end
    end
end
...
switch paramName
    ...
    case 'printLevel',	param = 3;  % verbal dispaly
    ...
    case 'newExId',     param = 'EX_%met(%comp)';  % rxn IDs for new community exchange reactions
    case 'newUtId',     param = 'UT_%met(%comp)';  % rxn IDs for new community uptake reactions
    case 'newCompId',   param = 'u';  % compartment ID for the new community compartment
    case 'compSuffix',  param = '[%comp]';  % suffix added to the new community met IDs
