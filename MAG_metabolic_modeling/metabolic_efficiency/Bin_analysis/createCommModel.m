...
%Create a community COBRA model. The model has an extra compartment with default name [u]  
...
%   (below are optional, but recommended to provide, 'sp' originally for 'species')
...
%               each model in 'modelCell'
%   (below are more detailed configuration, added for flexibility)
...
%     metExId:  Identifier for compartment for extracellular metabolites 
%               including suffix and prefix (default '[e]').
%               If input is the empty string (''), find all metabolites that have exchange reactions
%     newCompId: Identifier for the new community compartment (default 'u')
%     compSuffix: suffix added to the new community met IDs, 
%                 entered as a formatted string wiith %comp for 'newCompId' (default '[%comp]')
%     newExId:  Rxn IDs for newly added community exchange (or export if 'sepUtEx' = true) reactions, 
%               inputted as a formatted string with %met for met IDs (without compartment suffix) 
%               and %comp for 'newCompId' [default 'EX_%met(%comp)']
%     newUtId:  Rxn IDs for newly added community uptake reactions,
%               inputted as a formatted string with %met for met IDs (without compartment suffix) 
%               and %comp for 'newCompId' [used only if 'sepUtEx' = true, default 'UT_%met(%comp)']
...
%get parameters
[spAbbr,spName,spBm,spATPM,metExId,newCompId,compSuffix,newExId,newUtId, ...
    rxnField,metField,geneField,compField,sepUtEx,addExRxns,verbFlag] = getCobraComParams(...
    {'spAbbr','spName','spBm','spATPM','metExId','newCompId','compSuffix','newExId','newUtId', ...
    'rxnField','metField','geneField','compField','sepUtEx','addExRxns','verbFlag'}, options);
...
elseif iscell(spBm)
    ...
    spBm0 = cell(nSp, 1);
    for j = 1:nSp
        spBm0{j} = modelCell{j}.rxns{spBm(j)};
    end
end
...
compSuffix = strrep(compSuffix, '%comp', newCompId);
newExId = strrep(strrep(newExId, '%comp', newCompId), '%met', '%s');
newUtId = strrep(strrep(newUtId, '%comp', newCompId), '%met', '%s');
    ...
    if addExRxns
        ...
        if ~isempty(metExWoER)
            ...
            for jF = 1:numel(field)
                if rxnFieldL(jF)
                    if ~isfield(modelCell{j}, field{jF}) || isempty(modelCell{j}.(field{jF}))
                        if fieldNumeric(jF)
                            modelCell{j}.(field{jF}) = zeros(colJ, 1);
                        elseif fieldCell(jF)
                            modelCell{j}.(field{jF}) = repmat({''}, colJ, 1);
                        elseif fieldStruct(jF)
                            modelCell{j}.(field{jF}) = repmat(struct(), colJ, 1);
                        end
                    elseif size(modelCell{j}.(field{jF}), 1) < colJ
                        if fieldNumeric(jF)
                            modelCell{j}.(field{jF})((end + 1):colJ, :) = 0;
                        elseif fieldCell(jF)
                            modelCell{j}.(field{jF})((end + 1):colJ, :) = {''};
                        elseif fieldStruct(jF)
                            tmp = fieldnames(modelCell{j}.(field{jF}));
                            tmp = reshape([tmp, repmat({[]}, numel(tmp), 1)]', numel(tmp) * 2, 1);
                            modelCell{j}.(field{jF})((end + 1):colJ, :) = struct(tmp{:});
                        end
                    end
                end
            end
...
if sepUtEx
    ...
    rxns = [rxns; cellfun(@(x) sprintf(newUtId, x), metsCom(wtExCom), 'UniformOutput', false); ...
        cellfun(@(x) sprintf(newExId, x), metsCom(wtExCom), 'UniformOutput', false)];
else
    rowS = [rowS; (1:mCom)'];
    ...
    %names of community exchange reactions
    rxns = [rxns; cellfun(@(x) sprintf(newExId, x), metsCom(wtExCom), 'UniformOutput', false)];
end
b = [b; zeros(mCom, 1)];
metSps = [metSps; zeros(mCom, 1)];
%names of community metabolites
mets = [mets; strcat(metsCom(wtExCom),compSuffix)];

comps = [comps; {newCompId}];
        ...
        for j = 1:mCom
            ...
            if sepUtEx
                if maxLength > 0
                    ...
                    if ~isempty(strfind(rxnNameJ, 'exchange'))  % change to 'contains' when this becomes obsolete
