...
if ~revFlag
    ...
    if isfield(infoCom, 'EXhost') && ~isempty(infoCom.EXhost)
    ...
    if isfield(infoCom, 'Mhost') && ~isempty(infoCom.Mhost)
    ...
    if isfield(infoCom, 'geneSps')
        [~, indCom.geneSps] = ismember(infoCom.geneSps, infoCom.spAbbr);
    end
    if isfield(infoCom, 'compSps')
        [~, indCom.compSps] = ismember(infoCom.compSps, infoCom.spAbbr);
    end
else
    ...
    if isfield(infoCom, 'geneSps')
        indCom.geneSps = repmat({'com'}, numel(modelCom.genes), 1);
        indCom.geneSps(infoCom.geneSps > 0) = spAbbr(infoCom.geneSps(infoCom.geneSps > 0));
    end
    if isfield(infoCom, 'compSps')
        indCom.compSps = repmat({'com'}, numel(modelCom.comps), 1);
        indCom.compSps(infoCom.compSps > 0) = spAbbr(infoCom.compSps(infoCom.compSps > 0));
    end
end
end