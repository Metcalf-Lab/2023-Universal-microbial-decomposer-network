...
day0 = read.csv("../Day0_SHSU.csv") # Importing the organismal data
early = read.csv("../Early_SHSU.csv") # Importing the organismal data
active = read.csv("../Active_SHSU.csv") # Importing the organismal
advanced = read.csv("../Advanced_SHSU.csv") # Importing the organismal
...
#saveRDS(bMNTD, "SHSU_bMNTD.rds")
bMNTD = readRDS(file="SHSU_bMNTD.rds")
...
shsu_rand1 = readRDS(file="SHSU_bMNTD_rand1.rds")
shsu_rand2 = readRDS(file="SHSU_bMNTD_rand2.rds")
shsu_rand3 = readRDS(file="SHSU_bMNTD_rand3.rds")
shsu_rand4 = readRDS(file="SHSU_bMNTD_rand4.rds")
bMNTD.rand = abind(shsu_rand1,shsu_rand2,shsu_rand3,shsu_rand4)

# sanity check that random arrays combined and match sample array
(sum(dim(shsu_rand1)[3],dim(shsu_rand2)[3],dim(shsu_rand3)[3],dim(shsu_rand4)[3]) + sum(dim(bMNTD.rand)[3])) / 2 == 999
dim(shsu_rand1)[1] == dim(shsu_rand2)[1] | dim(shsu_rand2)[1] == dim(shsu_rand3)[1] | dim(shsu_rand3)[1] == dim(shsu_rand4)[1]
...
write.csv(bNTI, "SHSU_Community_bNTI_TotalCounts.csv", quote = F)
