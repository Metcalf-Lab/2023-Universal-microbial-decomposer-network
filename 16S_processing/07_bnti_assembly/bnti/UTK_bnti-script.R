...
day0 = read.csv("../Day0_UTK.csv") # Importing the organismal data
early = read.csv("../Early_UTK.csv") # Importing the organismal data
active = read.csv("../Active_UTK.csv") # Importing the organismal
advanced = read.csv("../Advanced_UTK.csv") # Importing the organismal
...
#saveRDS(bMNTD, "UTK_bMNTD.rds")
bMNTD = readRDS(file="UTK_bMNTD.rds")
...
UTK_rand1 = readRDS(file="UTK_bMNTD_rand1.rds")
UTK_rand2 = readRDS(file="UTK_bMNTD_rand2.rds")
UTK_rand3 = readRDS(file="UTK_bMNTD_rand3.rds")
UTK_rand4 = readRDS(file="UTK_bMNTD_rand4.rds")
bMNTD.rand = abind(UTK_rand1,UTK_rand2,UTK_rand3,UTK_rand4)

# sanity check that random arrays combined and match sample array
(sum(dim(UTK_rand1)[3],dim(UTK_rand2)[3],dim(UTK_rand3)[3],dim(UTK_rand4)[3]) + sum(dim(bMNTD.rand)[3])) / 2 == 999
dim(UTK_rand1)[1] == dim(UTK_rand2)[1] | dim(UTK_rand2)[1] == dim(UTK_rand3)[1] | dim(UTK_rand3)[1] == dim(UTK_rand4)[1]
...
write.csv(bNTI, "UTK_Community_bNTI_TotalCounts.csv", quote = F)
